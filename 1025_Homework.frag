...
void main() {
 ...
 {  
    ...
    float weight = smoothstep(0.336, -0.096, -uv.y);
    float freq = 0.7+float(index)*0.356;
    float noise = gnoise(uv_flip*freq+vec2(-0.796*u_time, 0.536*u_time)/*移動*/)*0.220*weight;
    //float model_dist = abs(heart(uv_flip, 1.040)+noise);
    
    //float star5 = sdStar5(uv,0.468,0.972)+noise;
    float moon = sdMoon(uv,0.016,0.680,0.528)+noise;
    ...
    gl_FragColor = vec4((vec3(result)+fog)/**dir*/*vec3(1.000,0.760,0.769)*.5/*強度*/,1.0);
}
